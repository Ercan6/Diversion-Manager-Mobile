...
const MitigatingFactors = ({ navigation }) => {
  ...
  // Function to update mitigating factors in global state context
  const setSelectedMitigatingFactors = (value) => {
    const updatedMitigatingFactors = {
      ...gravityscore,
      mitigatingfactors: value,
    };
  ...
  const nextPage = () => {
    if (mitigatingFactors.trim() !== "") {
      ...
      navigation.navigate("Aggrevating Factors"); //Navigate to next View
    } else {
      ...
      <View style={styles.background}>
        ...
        >
          <TextInput
            ...
            onChangeText={(value) => {
              setMitigatingFactors(value); // Update local input state
            }}
          />
        </ScrollView>
